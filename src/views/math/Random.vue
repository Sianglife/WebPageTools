        ...
        <div class="row"
            ...
            <div class="col" style="height: 16em;border-color: var(--bs-gray-500);border-right-style: dotted;">
                <div class="row" style="margin: 0.5em;height: 3em;">
                    <div class="col" style="width: 100%;text-align: center;"><span style="font-size: 1.5em;width: 25%;">最小:
                        </span><input v-on:change="listUpdate" v-model="min" type="number"
                            style="font-size: 1.5em;height: 2em;width: 75%;" :max="max" /></div>
                </div>
                <div class="row" style="margin: 0.5em;height: 3em;">
                    <div class="col" style="width: 100%;text-align: center;"><span style="font-size: 1.5em;width: 25%;">最大:
                        </span><input @change="listUpdate" v-model="max" type="number"
                            style="font-size: 1.5em;height: 2em;width: 75%;" :min="min" /></div>
                </div>
                <div class="row" style="margin: 0.5em;height: 3em;">
                    <div class="col" style="width: 100%;text-align: center;"><span style="font-size: 1.5em;width: 25%;">數量:
                        </span><input @change="checkCount" v-model="count" type="number"
                            style="font-size: 1.5em;height: 2em;width: 75%;" min="1" :max="max" /></div>
                </div>
                <div class="row" style="margin: 0.5em;height: 1.5em;">
                    <div class="col text-center d-xxl-flex align-items-xxl-center" style="width: 100%;text-align: center;">
                        <label class="col-form-label" style="height: 1.5em;font-size: 1em;"><input class="form-check-input"
                                v-model="allowRepeat" type="checkbox">允許重複</label></div>
                </div>
                <div class="row">
                    <div class="col" style="width: 100%;text-align: center;height: 34px;margin: 0em;"><button
                            v-on:click="random" class="btn btn-secondary" type="button" style="width: 72%;">抽！</button>
                    </div>
                </div>
            ...
            <li class="list-group-item" v-for="item in result" :key="item">
                <div class="row">
                    ...
                    <div class="col" style="text-align: right;"><button
                            :class="(item[2]) ? `btn btn-primary` : `btn btn-primary disabled`" type="button"
                            :id="`btnRemove${item[0]}`" v-on:click="removeListItem(item)">從待選名單移除</button></div>
                </div>
            </li>
        </ul>
    </div>
